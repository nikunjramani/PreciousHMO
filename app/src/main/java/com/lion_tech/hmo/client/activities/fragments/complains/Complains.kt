...
class Complains : Fragment() {
    ...
    override fun onCreateView(
        ...
        complainsViewModel =
            ViewModelProvider(this).get(ComplainsViewModel::class.java)
        val root = inflater.inflate(R.layout.fragment_complains, container, false)
        ...
        AppLevelData.complaintFragmentObject = this
        return root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (!AppLevelData.verifyAvailableNetwork(context as DashboardClient)) {
            ...
            complainsViewModel.getClientComplains(activity as DashboardClient)
        }

        rvAdapter = ComplaintRvAdapter(activity as DashboardClient)

        val rvComplaint = view.findViewById<RecyclerView>(R.id.rvComplaint)
        rvComplaint.layoutManager = LinearLayoutManager(activity as DashboardClient)
        rvComplaint.adapter = rvAdapter

        complainsViewModel.complainDataList.observe(viewLifecycleOwner, Observer {
            complaintList=it
            rvAdapter.refreshAdapter(it)
            AppLevelData.clientComplaintCount=it.size
    ...
    fun showComplaintDialog() {
        val view =
            LayoutInflater.from(activity as DashboardClient)
        ...
        btnAddComplaint.setOnClickListener {
            ...
            if (!AppLevelData.verifyAvailableNetwork(context as DashboardClient)) {
                ...
                complainsViewModel.addClientTicket(etSubject.text.toString(), etDetail.text.toString())
            }
    ...
    fun filterCompaintOnDate(date: String) {
        var compliantFilterList= complaintList.filter { complaintModel -> complaintModel.ticketDate.contains(date) }
